...
public banmod_online(id)
{
	...
	if(contain(g_ip_port,":") == -1)
		get_user_ip(0, g_ip_port, charsmax(g_ip_port), 0); // Takes in the whole IP:port string.. (0 is always the server)

	if ( get_pcvar_num(pcvar_debug) >= 1 )
	{
		server_print("[AMXBans] The server IP:PORT is: %s", g_ip_port);
		log_amx("[AMXBans] The server IP:PORT is: %s", g_ip_port);
	}
	new pquery[1024];
	formatex(pquery, charsmax(pquery), "SELECT `motd_delay` FROM `%s%s` WHERE address = '%s'", g_dbPrefix, TBL_SERVERINFO, g_ip_port);
